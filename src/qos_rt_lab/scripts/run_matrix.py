...
# matrix spec: "paramA,paramB;A1|A2,B1|B2"
...
for i, cfg in enumerate(runs, 1):
    ...
    print("[*] Running", i, "of", len(runs), ":", " ".join(cmd))
    ...
    summary_path = os.path.join(args.out_dir, f"qos_rt_{run_id}.json")
    if os.path.exists(summary_path):
        with open(summary_path) as f:
            data = json.load(f)
        row = {"run_id": run_id, **cfg, **data}
        rows.append(row)
    else:
